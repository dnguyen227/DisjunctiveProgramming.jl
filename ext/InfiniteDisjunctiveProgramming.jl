...
################################################################################
# Reuses the finite MBM infrastructure by overriding: create_submodel
# (build mini InfiniteModel + transcribe to flat JuMP model),
# _prepare_objectives (expand infinite constraint into K flat objectives
# via _build_flat_map), and condense_values (interpolate flat values to
...
function DP.create_submodel(
    ...
    # 7. Remap fwd: original model var -> flat JuMP VarRef
...
function transcribe_to_flat(model::InfiniteOpt.InfiniteModel)
    ...
    for v in DP.collect_all_vars(model)
        ...
        fwd[v] = isempty(vprefs) ? [tv] : vec(tv)
    end
    return flat, fwd
end
...
function DP.build_cp_subproblem(
    ...
    for v in dec_vars
        ...
        fwd[v] = [copy_map[tv] for tv in tr_fwd[v]]
    end
    sub = DP.GDPSubmodel(sub_copy, dec_vars, fwd)
...
function DP.setup_rbm(
    ...
    rBM = DP.build_cp_subproblem(model, dec_vars, DP.BigM(method.M_value), method)
