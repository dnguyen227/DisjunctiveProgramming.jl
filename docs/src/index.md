...
4. Multiple Big-M: A separate big-M value is computed for each constraint-disjunct pair by optimizing over the other disjunct's region, giving tighter coefficients than a single big-M at no cost in program size. The [`MBM`](@ref) struct is used, and requires an optimizer for the subproblems.

5. P-Split: Variables are partitioned into groups and disaggregated by group, giving a formulation whose strength and size sit between Big-M and Hull. The [`PSplit`](@ref) struct is used.

6. Cutting Planes: Valid cuts for the hull are generated by solving a sequence of separation problems, after which a final reformulation is applied to the strengthened model. The [`CuttingPlanes`](@ref) struct is used, and requires an optimizer for the separation problems.

See [Solution Methods](@ref methods_guide) in the User Guide for a comparison of the methods and their parameters.
