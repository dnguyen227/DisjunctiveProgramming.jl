...
# create_submodel lines 308 (finite var), 325 (set_integer),
...
function test_cutting_planes_infinite_simple()
    model = InfiniteGDPModel(HiGHS.Optimizer)
    set_silent(model)
...
function test_cutting_planes_infinite_two_disj()
    model = InfiniteGDPModel(HiGHS.Optimizer)
    set_silent(model)
...
function test_transcribe_accessors()
    model = InfiniteGDPModel(HiGHS.Optimizer)
    set_silent(model)
    ...
    tmap = transcribe(model)

    # jump_model
    flat = jump_model(tmap)
    @test flat isa JuMP.Model

    # support_values
    sups = support_values(tmap)
    @test haskey(sups, t)
    @test length(sups[t]) == 5
    @test sups[t][1] ≈ 0.0
    @test sups[t][end] ≈ 1.0

    # transcribed_variable: infinite → K vars
    x_arr = transcribed_variable(tmap, x)
    @test x_arr isa Vector{JuMP.VariableRef}
    @test length(x_arr) == 5

    # transcribed_variable: finite → 1 var
    z_arr = transcribed_variable(tmap, z)
    @test length(z_arr) == 1

    # infinite_variable: inverse lookup
    inf_var, k = infinite_variable(tmap, x_arr[3])
    @test inf_var == x
    @test k == 3
    fin_var, fk = infinite_variable(tmap, z_arr[1])
    @test fin_var == z
    @test fk == 1
end

function test_transcribe_accessors_error()
    model = InfiniteGDPModel(HiGHS.Optimizer)
    set_silent(model)
    @infinite_parameter(model, t ∈ [0, 1], num_supports = 5)
    @variable(model, 0 <= x <= 10, Infinite(t))
    ...
    # transcribed_variable with unknown var
    model2 = InfiniteGDPModel()
    @variable(model2, q)
    @test_throws ErrorException transcribed_variable(tmap, q)

    # infinite_variable with unknown flat var
    dummy = JuMP.@variable(jump_model(tmap))
    @test_throws ErrorException infinite_variable(tmap, dummy)
end

function test_transcribe_multiparameter()
    model = InfiniteGDPModel(HiGHS.Optimizer)
    set_silent(model)

    @infinite_parameter(model, t ∈ [0, 1], num_supports = 4)
    @infinite_parameter(model, s ∈ [0, 2], num_supports = 3)
    @variable(model, 0 <= v <= 10, Infinite(t, s))
    @variable(model, 0 <= w <= 10, Infinite(t))
    @variable(model, z)

    @variable(model, Y[1:2], InfiniteLogical(t, s))
    @constraint(model, v >= 5, Disjunct(Y[1]))
    @constraint(model, v <= 3, Disjunct(Y[2]))
    @disjunction(model, Y)
    @objective(model, Min, ∫(∫(v, t), s))

    tmap = transcribe(model)

    # support_values: both params
    sups = support_values(tmap)
    @test haskey(sups, t)
    @test haskey(sups, s)
    @test length(sups[t]) == 4
    @test length(sups[s]) == 3

    # v(t,s) → 4*3 = 12
    v_arr = transcribed_variable(tmap, v)
    @test length(v_arr) == 12

    # w(t) → 4
    w_arr = transcribed_variable(tmap, w)
    @test length(w_arr) == 4
    ...
    # Inverse lookup: multiparameter var
    inf_v, idx = infinite_variable(tmap, v_arr[1])
    @test inf_v == v
    @test idx == 1
    # Inverse lookup: single-param var in
    # multiparameter model
    inf_w, widx = infinite_variable(tmap, w_arr[2])
    @test inf_w == w
    @test widx == 2
end

function test_lift_constraint_geq()
    model = InfiniteGDPModel(HiGHS.Optimizer)
    set_silent(model)
    ...
    flat = jump_model(tmap)
    sups = support_values(tmap)
    K = prod(length(v) for v in values(sups))

    # x >= 4
    lift_constraint(tmap, [(x, ones(K))], :>=, fill(4.0, K))

    # Flat model respects the cut
    set_optimizer(flat, HiGHS.Optimizer)
    set_silent(flat)
    optimize!(flat)
    x_arr = transcribed_variable(tmap, x)
    for v in x_arr
        @test value(v) >= 4.0 - 1e-6
    end

    # InfiniteModel also respects the cut
    optimize!(model, gdp_method = BigM())
    @test termination_status(model) in
        [MOI.OPTIMAL, MOI.LOCALLY_SOLVED]
    @test objective_value(model) >= 4.0 - 1e-6
end

function test_lift_constraint_leq()
    model = InfiniteGDPModel(HiGHS.Optimizer)
    set_silent(model)
    ...
    # x <= 6
    lift_constraint(tmap, [(x, ones(K))], :<=, fill(6.0, K))
    ...
    x_arr = transcribed_variable(tmap, x)
    for v in x_arr
        @test value(v) <= 6.0 + 1e-6
    end
end

function test_lift_constraint_eq()
    model = InfiniteGDPModel(HiGHS.Optimizer)
    set_silent(model)
    ...
    # x == 7
    lift_constraint(tmap, [(x, ones(K))], :(==), fill(7.0, K))
    ...
    x_arr = transcribed_variable(tmap, x)
    for v in x_arr
        @test value(v) ≈ 7.0 atol = 1e-6
    end
end

function test_lift_constraint_multiparameter()
    model = InfiniteGDPModel(HiGHS.Optimizer)
    set_silent(model)
    ...
    flat = jump_model(tmap)
    sups = support_values(tmap)
    K = prod(length(sv) for sv in values(sups))

    # Cut with mixed infinite + finite vars:
    # v + w + z >= 6
    lift_constraint(tmap,
        [(v, ones(K)), (w, ones(K)), (z, ones(K))],
        :>=, fill(6.0, K))
    ...
    # Verify cut holds at each support point
    for k in 1:K
        # v has 12 flat vars (4*3), w has 4, z has 1
        # _global_to_var_index handles the mapping
        vk = min(k, length(v_arr))
        wk = min(k, length(w_arr))
        @test value(v_arr[vk]) + value(w_arr[wk]) + value(z_arr[1]) >= 6.0 - 1e-6
    end
end

function test_cutting_planes_with_cuts()
...
function test_cutting_planes_multiparameter()
    model = InfiniteGDPModel(HiGHS.Optimizer)
    set_silent(model)
...
@testset "InfiniteDisjunctiveProgramming" begin
    ...
    @testset "Cutting Planes" begin
        test_cutting_planes_infinite_simple()
        test_cutting_planes_infinite_two_disj()
        test_cutting_planes_with_cuts()
        test_cutting_planes_multiparameter()
    end

    @testset "Transcribe Accessors" begin
        test_transcribe_accessors()
        test_transcribe_accessors_error()
        test_transcribe_multiparameter()
    end

    @testset "Lift Constraint" begin
        test_lift_constraint_geq()
        test_lift_constraint_leq()
        test_lift_constraint_eq()
        test_lift_constraint_multiparameter()
    end
